    ...
    build: {
      ...
      rollupOptions: {
        ...
        output: {
           ...
           */
          chunkFileNames: 'identity/js/[name]-[hash].js',
          /**
           ...
           */
          entryFileNames: 'identity/js/[name]-[hash].js',
          ...
          assetFileNames: ({ name }: { name: string }) => {
            ...
            if (/\.(gif|jpe?g|png|svg)$/.test(name ?? '')) {
              return 'identity/assets/images/[name]-[hash][extname]';
            }

            // Move files which end with css to assets/css
            if (/\.css$/.test(name ?? '')) {
              return 'identity/assets/css/[name]-[hash][extname]';
            }

            if (/\.(ttf|woff|woff2)$/.test(name ?? '')) {
              return 'identity/assets/font/[name]-[hash][extname]';
            }

            // Default value
            // ref: https://rollupjs.org/guide/en/#outputassetfilenames
            return 'identity/[name]-[hash][extname]';
          },
        },
      },
    },
